...
int get_if_name(void)
{
	...
	if (cfg.ifname[0] == '\0') {
		if (get_output_if(&ctx.remote, &output_if_addr) == 0) {
			known_output_if = 1;
			if (cfg.opt_debug)
	...
	for (; ifrp < ifend; ifrp++) {
		...
		} else if (cfg.ifname[0] != '\0' && !strstr(ifr.ifr_name, cfg.ifname)) {
			if (cfg.opt_debug)
		...
		strlcpy(cfg.ifname, ifr.ifr_name, 1024);
		...
		memcpy(&sa, &ifr.ifr_addr,
			sizeof(struct sockaddr_in));
		strlcpy(ctx.ifstraddr, inet_ntoa(sa.sin_addr), 1024);
		...
		else
		{
			...
			ctx.h_if_mtu = ifr.ifr_metric;
#else
			ctx.h_if_mtu = ifr.ifr_mtu;
#endif
		}
		close(fd);
		return 0;
	...
	strlcpy(cfg.ifname, "lo", 1024);
	strlcpy(ctx.ifstraddr, "127.0.0.1", 1024);
	ctx.h_if_mtu = 1500;
...
int get_if_name(void)
{
	...
	if(cfg.ifname[0] == 0) {
		...
		strlcpy(saved_ifname,cfg.ifname,24);
	}

	/* get interface information */
        for (ifa = ifap; ifa; ifa = ifa->ifa_next) {

		if (cfg.opt_debug) printf("\n DEBUG: if %s: ", ifa->ifa_name);

		/* print if the data structure is null or not */
		if (ifa->ifa_data) {
			if(cfg.opt_debug) printf("DEBUG: (struct DATA) "); }
		else
			if(cfg.opt_debug) printf("DEBUG: (struct DATA is NULL) ");
		...
		if (ifa->ifa_addr->sa_family == AF_LINK) {
			...
			strlcpy(cfg.ifname,ifa->ifa_name,1024);
...
/* I don't know why NetBSD behavior is not the same */
			...
			if (ioctl(s, SIOCGIFMTU, (caddr_t)&ifr) < 0) ctx.h_if_mtu = 0;
			else ctx.h_if_mtu = ifr.ifr_mtu;
			close(s);
#else
			if( ifa->ifa_data )
				ctx.h_if_mtu = ((struct if_data *)ifa->ifa_data)->ifi_mtu;
			else {
				ctx.h_if_mtu = 1500;
		...
		if (ifa->ifa_addr->sa_family == AF_INET) {
			...
			if(cfg.opt_debug) printf("OK\n");

			strlcpy(ctx.ifstraddr,
